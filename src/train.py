...
def main():
    ...
    # Directories
    root_dir = "/mnt/data/Shared/"
    proj_dir = "ClipMeetsGAN"
    data_dir = "image_seq_dataset"
    clip_ds_dir = "clip_data/clip_seq_dataset"
    clip_root = os.path.join(root_dir, clip_ds_dir)
    root = os.path.join(root_dir, proj_dir, data_dir)
    ...
    model = ANOMALY_CLIP_LSTM().to(device)
    ...
    annotation_file = os.path.join(root, "full_anomaly_annotations.txt")
    ...
    dataset = VideoFrameDataset(
                ...
                transform=transforms, #ImglistToTensor(), If you want a dataset of tensors
                random_shift=True,
                test_mode=False,
                image=True
        )   
    ...
    # print(len(dataset))
    # import sys; sys.exit()
    train_dataset, valid_dataset = torch.utils.data.random_split(dataset, [1425, 475], 
