        ...
        for seg_ind in indices:
            ...
            for i in range(self.frames_per_segment):
                if self.image:
                    ...
                    with open(os.path.join(record.path, self.imagefile_template.format(frame_index)), 'r') as f:
                        embedding = f.readline()
                        embedding = embedding[2:-2].split(', ')
                        embedding = [float(x) for x in embedding]
                        seg_img = torch.tensor(embedding).unflatten(0,(1,512))
